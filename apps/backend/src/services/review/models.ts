...
/**
 * Reads the accepted reasoning efforts out of a model description.
 *
 * @param model - The model as the provider describes it.
 * @returns The accepted levels, or every level when the model says nothing.
 *
 * @remarks
 * Falling back to every level rather than to none keeps a model usable when the
 * provider adds a shape this does not know. The request then fails as it does
...
/**
 * Decides whether a model can run a check at all.
 *
 * @param model - The model as the provider describes it.
