...
function parseMarkdown(text: string): string {
  ...
  // marked v5+ returns string synchronously unless async extensions are used
...
export async function renderEmailTemplate(
  ...
  const headerHtml = template.headerText
    ? parseMarkdown(interpolate(template.headerText, variables))
    : null;

  const bodyHtml = parseMarkdown(interpolate(template.bodyText, variables));

  const footerHtml = template.footerText
    ? parseMarkdown(interpolate(template.footerText, variables))
    : null;

  const rows: string[] = [];

  // Header banner (conditional)
  if (template.headerBannerUrl) {
    rows.push(
      `<tr><td><img src="${template.headerBannerUrl}" width="560" alt="" style="display:block;width:100%;border-radius:8px 8px 0 0;"></td></tr>`,
    );
  }
  ...
  // Footer banner (conditional)
  if (template.footerBannerUrl) {
    rows.push(
      `<tr><td><img src="${template.footerBannerUrl}" width="560" alt="" style="display:block;width:100%;border-radius:0 0 8px 8px;"></td></tr>`,
    );
  }
