...
| Modell                                  | `claude-opus-5` | Zur Auswahl stehen die Modelle, die Anthropic meldet und die eine Prüfung auch ausführen können. Wird an jeder Prüfung mitgeschrieben. |
...
Steht die Verbindung, liest ein Hintergrundlauf alle sechs Stunden die Eingänge und trägt ein, was diesem Projekt gehört. Was das ist, entscheidet eine Positivliste und keine Ausschlussregel: entweder trägt der Verwendungszweck eine Referenz, die das Sponsorenformular nach ISO 11649 ausgegeben hat und deren Prüfziffern noch stimmen, oder er nennt den Host der Seite. Alles andere ist auf einem privaten Konto fremdes Leben und verlässt die lesende Funktion, ohne gespeichert, gezählt oder geloggt zu werden. Vom Zahler wird der Name übernommen und sonst nichts; die Anschrift, die die Schnittstelle daneben liefert, kommt gar nicht erst durch das Schema. Der Verwendungszweck wird nie im Klartext abgelegt.

Wie oft gelesen werden darf, steht in Artikel 36 Absatz 5 der Delegierten Verordnung (EU) 2018/389: viermal in 24 Stunden, solange der Kontoinhaber nicht selbst fragt. Der Zähler kommt aus der Tabelle `bank_account_reads` statt aus einem Timer, denn ein Deployment startet einen Timer neu und lässt kurz zwei Container mit je einem laufen. `POST /api/v1/admin/bank-connection/sync` löst einen Lauf von Hand aus und zählt getrennt, weil das der Fall aus Buchstabe a ist. Er nimmt optional `from` als Tag im Format `YYYY-MM-DD` und liest ab dort statt über sein eigenes Fenster. Wie weit eine Bank zurückgeht, entscheidet sie selbst; einen Zeitraum, den sie ablehnt, meldet der Lauf als abgelehnte Anfrage.

Ein Lauf über Tage, die schon gelesen wurden, trägt nach, was den gespeicherten Zeilen fehlt, und lässt alles andere stehen. Nur ein leeres Feld wird gefüllt, denn was jemand von Hand eingetragen hat, ist besser als der Auszug. Solche Zeilen zählt der Lauf getrennt von den übersprungenen, sonst sieht ein Lauf, der etwas repariert hat, aus wie einer, der nichts getan hat.
...
Auf der Seite Spendeneingänge steht eine Karte mit Bank, Konto, Ablaufdatum und letztem Lauf. Sie steht dort, weil dort die Zeilen liegen, die sie erzeugt: wer eine Zahlung vermisst, sucht in der Liste und nicht in einer Auswertung. Lesen darf diese Karte jeder Admin. Den Fussbereich mit den beiden Knöpfen und dem Startdatum sieht nur der Owner, denn alles darin greift auf das Konto zu.
