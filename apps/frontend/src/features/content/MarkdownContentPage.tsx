...
export function MarkdownContentPage({ slug, title, canonicalPath, description, subtitle }: Props) {
  usePageMeta({ title, canonicalPath, description });

  const { data: page } = useContentPage(slug);

  return (
    <PageLayout>
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-14">
        <h1 className="font-serif text-3xl font-semibold text-stone-900 mb-2">{title}</h1>
