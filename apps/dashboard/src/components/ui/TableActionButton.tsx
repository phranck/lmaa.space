...
export function TableActionButton({
  ...
  return (
    <button
      type="button"
      className={`h-9 px-3 flex items-center gap-2 border rounded-control text-sm transition-colors disabled:opacity-50 ${variantClasses[variant]}${className ? ` ${className}` : ""}`}
      {...rest}
    >
      {icon}
      {label}
